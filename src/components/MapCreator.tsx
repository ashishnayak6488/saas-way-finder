...
// const MapCreator: React.FC<MapCreatorProps> = ({ selectedMap, onClose }) => {
//     // Core state
...
//           console.log("Loaded buildings from storage:", savedBuildings);
...
//         const floor = selectedBuilding.floors.find(f => f.floor_id === selectedFloorId);
...
//         if (!isDesignMode || isEditMode || isPreviewMode || isTagMode || isVerticalTagMode) {
//           return;
//         }
      
//         // Check if clicking near a vertical connector for multi-floor navigation
//         if (isCreatingMultiFloorPath || isDesignMode) {
//           const clickThreshold = 0.03; // 3% of canvas size
          
//           const nearbyConnector = verticalConnectors.find((connector) => {
//             if (connector.floorId !== selectedFloor?.floor_id) return false;
            
//             const distance = Math.sqrt(
//               Math.pow(connector.x - x, 2) + Math.pow(connector.y - y, 2)
//             );
...
//           if (nearbyConnector) {
//             // Handle connector click for floor transition
//             if (isCreatingMultiFloorPath && selectedBuilding) {
//               // Find connected floors for this connector
//               const connectedFloors = selectedBuilding.floors.filter((floor) =>
//                 verticalConnectors.some(
//                   (c) => c.sharedId === nearbyConnector.sharedId && c.floorId === floor.floor_id
//                 )
//               );
      
//               if (connectedFloors.length > 1) {
//                 // Show floor selection or automatically switch to the other floor
//                 const otherFloors = connectedFloors.filter(
//                   (f) => f.floor_id !== selectedFloor?.floor_id
//                 );
                
//                 if (otherFloors.length === 1) {
//                   // Automatically switch to the other floor
//                   handleFloorTransition(otherFloors[0], nearbyConnector);
//                 } else if (otherFloors.length > 1) {
//                   // Show selection dialog for multiple floors
//                   // For now, just switch to the first available floor
//                   handleFloorTransition(otherFloors[0], nearbyConnector);
//                 }
//               }
//             }
...
//             ? tags.filter((tag) => tag.floorId === selectedFloor.floor_id || !tag.floorId)
//             : tags;
...
//                 {/* Map Name */}
//                 <div>
//                   <Label htmlFor="map-name" className="text-sm font-medium text-gray-700 mb-2 block">
//                     Map Name
//                   </Label>
//                   <Input
//                     id="map-name"
...
//                 {/* Building Selection */}
//                 <div>
//                   <Label htmlFor="building-select" className="text-sm font-medium text-gray-700 mb-2 block">
//                     Select Building
//                   </Label>
...
//                 {/* Floor Selection */}
//                 <div>
//                   <Label htmlFor="floor-select" className="text-sm font-medium text-gray-700 mb-2 block">
//                     Select Floor
//                   </Label>
//                   <Select 
...
//                               (segment) => segment.floorId === selectedFloor?.floor_id
//                             );
//                           }
//                           return (
...
//                         <li>1. Click "Manage Buildings" to create a new building</li>
//                         <li>2. Add floors with map images to your building</li>
//                         <li>3. Select the building and floor to start editing</li>
//                         <li>4. Create paths, add location tags, and publish your map</li>
//                       </ol>
//                     </div>
//                   </div>
//                 </div>
...
//                 >
//                   <BuildingIcon className="h-4 w-4 mr-2" />
//                   {buildings.length === 0 ? "Create Building" : "Manage Buildings"}
//                 </Button>
...
const [showFloorSelectionDialog, setShowFloorSelectionDialog] = useState(false);
const [availableFloorsForTransition, setAvailableFloorsForTransition] = useState<Floor[]>([]);
const [selectedConnectorForTransition, setSelectedConnectorForTransition] = useState<VerticalConnector | null>(null);
...
// Load buildings on component mount
    ...
    //   // Add current path with connector point to segments
    //   const pathWithConnector = [
    //     ...currentPath,
    //     { x: sourceConnector.x, y: sourceConnector.y },
    //   ];
    ...
    //   } else {
    //     setIsConnectorPromptActive(false);
    //     alert(`Error: Could not find matching connector "${sourceConnector.name}" on ${targetFloor.label}. Please ensure the connector exists on the target floor.`);
    //   }
    // };
...
// Add this function to convert API data to TaggedLocation
const convertLocationDataToTaggedLocation = (locationData: LocationData): TaggedLocation => {
  return {
    ...
    isPublished: locationData.is_published,
    description: locationData.description,
  };
};

// Add this function to convert TaggedLocation to API format
const convertTaggedLocationToLocationData = (tag: TaggedLocation): CreateLocationRequest => {
  return {
    name: tag.name,
...
useEffect(() => {
  ...
  loadLocationsForFloor();
    ...
    const handleConnectorClick = (connector: VerticalConnector) => {
      ...
      const matchingConnectors = verticalConnectors.filter(
        (c) => c.sharedId === connector.sharedId && c.floorId !== selectedFloor.floor_id
      );
      ...
      const availableFloors = selectedBuilding.floors.filter((floor) =>
        matchingConnectors.some((c) => c.floorId === floor.floor_id)
      );
  ...
  console.log("Connectors on current floor:", 
  ...
  console.log("Connectors grouped by sharedId:", grouped);
};
    ...
    <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
      <div className="p-6">
        ...
        <p className="text-gray-600 mb-4">
          The connector "{selectedConnectorForTransition?.name}" is available on multiple floors. 
          Which floor would you like to continue your path on?
        </p>
...
const handleCreateTag = async (
  ...
  if (pendingShape && selectedFloor) {
    ...
    if (createdLocation) {
      ...
      // Update the handleEditTag function
const handleEditTag = async (updatedTag: TaggedLocation) => {
  ...
  if (updated) {
    ...
    setTags(tags.map((tag) => (tag.id === updatedTag.id ? convertedTag : tag)));
  }
};
      ...
      // Update the handleDeleteTag function
const handleDeleteTag = async (tagId: string) => {
  ...
  if (success) {
    ...
    setPaths(
      ...
      //     if (nearbyConnector) {
      //       console.log("Clicked on vertical connector:", nearbyConnector.name);
      ...
      //         console.log("Connected floors found:", connectedFloors.length);
      
      //         if (connectedFloors.length > 1) {
      //           // Get floors other than current floor
      //           const otherFloors = connectedFloors.filter(
      //             (f) => f.floor_id !== selectedFloor?.floor_id
      //           );
      ...
      //           if (otherFloors.length === 1) {
      //             // Automatically switch to the other floor
      //             console.log("Auto-switching to floor:", otherFloors[0].label);
      //             handleFloorTransition(otherFloors[0], nearbyConnector);
      //           } else if (otherFloors.length > 1) {
      //             // Show floor selection dialog
      //             console.log("Multiple floors available, showing selection dialog");
      //             setAvailableFloorsForTransition(otherFloors);
      //             setSelectedConnectorForTransition(nearbyConnector);
      //             setShowFloorSelectionDialog(true);
      //           }
      //         } else {
      //           alert(`No connected floors found for connector "${nearbyConnector.name}". Please ensure this connector exists on other floors.`);
      //         }
      //       }
      //       return; // Important: return here to prevent regular path point placement
      //     }
      //   }
      
      //   // Regular path point placement
      //   console.log("Placing regular path point at:", x, y);
      //   const newPath = [...currentPath, { x, y }];
      //   setUndoStack([...undoStack, currentPath]);
      //   setCurrentPath(newPath);
      // };
