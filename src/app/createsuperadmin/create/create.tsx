...
// Define props interfaces - Fixed to match actual usage
interface CreateOrganizationProps {
  onClose: () => void;
  addOrganization: (organization: Organization) => Promise<void>; // Fixed: should be async
  fetchOrganizations: () => Promise<void>;
}

interface CreateSuperAdminProps {
  onClose: () => void;
  addUserOrAdmin: (orgId: string, user: User) => Promise<void>; // Fixed: should be async
  orgId: string;
  role: string;
  allRole?: any[]; // Optional since not used in component
}
...
export const CreateOrganization: React.FC<CreateOrganizationProps> = ({
  onClose,
  addOrganization, // Fixed: now properly typed
  fetchOrganizations,
}) => {
  const [orgData, setOrgData] = useState<OrgData>({
    entity_type: "parent",
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target;
    const parsedValue =
      name === "address_id" ? (value === "" ? null : parseInt(value)) : value;
  ...
  const handleCreateOrganization = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addOrganization(newOrg); // Fixed: now properly awaited
      onClose();
    } catch (error: any) {
      console.error("Error creating organization:", error);
      if (
...
export const CreateSuperAdmin: React.FC<CreateSuperAdminProps> = ({
  ...
  const handleInitiateEmailVerification = () => {
    ...
    console.log("Initiating email verification for:", email); // Debug log
    setShowEmailVerification(true);
    setErrors((prev) => ({ ...prev, username: null })); // Clear any existing errors
  };
  ...
  const handleEmailVerified = () => {
    console.log("Email verified successfully for:", userData.username); // Debug log
    toast.success("Email verified successfully!");
    setIsEmailVerified(true);
    setShowEmailVerification(false);
    setErrors((prev) => ({ ...prev, username: null })); // Clear any existing errors
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      toast.success(`${role} created successfully`, { id: loadingToast });
      await addUserOrAdmin(orgId, newUser); // Fixed: now properly awaited
      setUserData({
        firstName: "",
        lastName: "",
        username: "",
              ...
              {showEmailVerification && !isEmailVerified && (
                <div className="mt-4">
                  ...
                  <div className="text-xs text-gray-400 mb-2 p-2 bg-yellow-50 rounded">
                    Debug: Email = "{userData.username}", Type = "email", Length
                    = {userData.username?.length || 0}
                  </div>

                  <OtpVerification
                    type="email"
                    value={userData.username?.trim() || ""} // Ensure it's trimmed and not undefined
                    onVerified={handleEmailVerified}
                    onCancel={() => {
                      setShowEmailVerification(false);
                      setIsEmailVerified(false);
