...
const Page: React.FC = () => {
  ...
  const confirmDeleteOrganization = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (success) {
        ...
        await fetchAllUsers(); // Refresh users after deleting organization
        toast.success("Organization deleted successfully");
      }
    } catch (error: any) {
      toast.error(`Failed to delete organization: ${error.message}`);
  ...
  const addUserOrAdmin = async (orgId: string, user: User) => {
    addUserOrAdminToOrg(organizations, setOrganizations, orgId, user);
    await fetchAllUsers(); // Refresh users after adding
  };
