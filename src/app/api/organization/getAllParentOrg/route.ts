...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const response: Response = await fetch(
      `${apiUrl}/v1/organization/getAllParentOrg`,
      {
        method: "GET",
        headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { message: errorData?.message || "Failed to fetch parent organizations" } as ErrorResponse,
    ...
    console.error("Get Parent Organizations error:", error);
