...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const response: Response = await fetch(
      `${apiUrl}/v1/organization/getPerentOrgOfSuperAdmin`,
      {
        method: "GET",
        headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { message: errorData?.message || "Failed to fetch super admin organizations" } as ErrorResponse,
    ...
    console.error("Get Super Admin Organizations error:", error);
