...
import { Navigation, MapPin, Route, Building as BuildingIcon } from 'lucide-react';
...
import { VerticalConnectorTagger, VerticalConnector } from '@/components/VerticalConnectorTagger';
...
const Index = () => {
  ...
  const handleCanvasClick = (x: number, y: number) => {
    ...
    if ((isDesignMode || isEditMode) && selectedFloor && selectedBuilding && !isConnectorPromptActive) {
      const floorConnectors = verticalConnectors.filter(c => c.floorId === selectedFloor.id);
      const clickedConnector = floorConnectors.find(connector => {
        ...
        } else if (connector.shape === 'rectangle' && connector.width && connector.height) {
          const avgSize = ((connector.width * canvasSize.width) + (connector.height * canvasSize.height)) / 2;
      ...
      if (clickedConnector && clickedConnector.id !== lastConnectorInteraction) {
  ...
  const handleConnectorClick = (connector: VerticalConnector) => {
    ...
    const matchingConnectors = verticalConnectors.filter(c => 
      c.sharedId === connector.sharedId && c.floorId !== selectedFloor.id
    );
    ...
    if (matchingConnectors.length === 0) {
      ...
      alert(`No matching connector "${connector.name}" found on other floors. Please ensure the connector exists on multiple floors with the same Shared ID.`);
      return;
    }
    ...
    if (selectedFloorLabel) {
      const targetFloor = availableFloors.find(f => 
        f.label.toLowerCase() === selectedFloorLabel.toLowerCase()
      );
  ...
  const handleFloorTransition = (targetFloor: Floor, sourceConnector: VerticalConnector) => {
    ...
    const pathWithConnector = [...currentPath, { x: sourceConnector.x, y: sourceConnector.y }];
    ...
    const targetConnector = verticalConnectors.find(c => 
  ...
  const handleShapeDrawn = (shape: Omit<TaggedLocation, 'id' | 'name' | 'category'>) => {
    setPendingShape(shape);
    setShowTagDialog(true);
  };

  const handleCreateTag = (name: string, category: string, floorId?: string) => {
    if (pendingShape) {
  ...
  const handleDeleteTag = (tagId: string) => {
    ...
    setPaths(paths.map(path => ({
      ...
      destinationTagId: path.destinationTagId === tagId ? undefined : path.destinationTagId
    })));
  };
  ...
  const handleSavePath = (source: string, destination: string) => {
    ...
    const sourceTag = tags.find(tag => tag.name.toLowerCase() === source.toLowerCase());
    const destinationTag = tags.find(tag => tag.name.toLowerCase() === destination.toLowerCase());
    ...
    if (isCreatingMultiFloorPath && multiFloorPathSegments.length > 0) {
      ...
      console.log('Saved multi-floor path with', allSegments.length, 'segments');
    } else {
      ...
      if (selectedPath) {
        const updatedPath = {
          ...
          name: source && destination ? `${source} to ${destination}` : selectedPath.name
        };
  ...
  const handleEditPath = (path: Path) => {
    ...
    if (path.isMultiFloor && path.segments) {
      ...
      const targetFloor = selectedBuilding?.floors.find(f => f.id === lastSegment.floorId);
      if (targetFloor) {
        setSelectedFloor(targetFloor);
        setMapImage(targetFloor.imageUrl);
      }
      ...
      if (path.floorId && selectedFloor?.id !== path.floorId) {
        const targetFloor = selectedBuilding?.floors.find(f => f.id === path.floorId);
        if (targetFloor) {
          setSelectedFloor(targetFloor);
          setMapImage(targetFloor.imageUrl);
        }
  ...
  const handleFloorAdd = (buildingId: string, floor: Omit<Floor, 'id' | 'createdAt'>) => {
    ...
    if (updatedBuilding) {
      ...
      const newFloor = updatedBuilding.floors[updatedBuilding.floors.length - 1];
  ...
  const handleFloorDelete = (buildingId: string, floorId: string) => {
    ...
    if (updatedBuilding) {
      ...
      if (selectedFloor?.id === floorId) {
        ...
        setSelectedFloor(remainingFloors.length > 0 ? remainingFloors[0] : null);
        setMapImage(remainingFloors.length > 0 ? remainingFloors[0].imageUrl : null);
      }
    }
  };

  const handleFloorReorder = (buildingId: string, reorderedFloors: Floor[]) => {
    const updatedBuildings = reorderFloorsInBuilding(buildings, buildingId, reorderedFloors);
    setBuildings(updatedBuildings);
  ...
  const handleVerticalShapeDrawn = (shape: Omit<VerticalConnector, 'id' | 'name' | 'type' | 'sharedId' | 'createdAt'>) => {
    setPendingVerticalShape(shape);
    setShowVerticalConnectorDialog(true);
  };

  const handleCreateVerticalConnector = (name: string, type: any, sharedId: string) => {
  ...
  const handleEditVerticalConnector = (updatedConnector: VerticalConnector) => {
    const updated = updateVerticalConnector(verticalConnectors, updatedConnector.id, updatedConnector);
  ...
  const handleRouteSelect = (path: Path | null, segmentIndex?: number) => {
    ...
    if (path.isMultiFloor && path.segments && typeof segmentIndex === 'number') {
      ...
      if (selectedFloor?.id !== targetSegment.floorId) {
        const targetFloor = selectedBuilding?.floors.find(f => f.id === targetSegment.floorId);
        if (targetFloor) {
          setSelectedFloor(targetFloor);
          setMapImage(targetFloor.imageUrl);
          
          console.log(`Switched to floor ${targetFloor.label} for segment ${segmentIndex + 1}`);
        }
      }
      ...
      if (isFirst) {
        console.log(`Showing first segment: ${path.source} to vertical connector`);
      } else if (isLast) {
        console.log(`Showing final segment: vertical connector to ${path.destination}`);
      } else {
      ...
      if (firstSegment && selectedFloor?.id !== firstSegment.floorId) {
        ...
        const startFloor = selectedBuilding?.floors.find(f => f.id === firstSegment.floorId);
        if (startFloor) {
      ...
      if (path.floorId && selectedFloor?.id !== path.floorId) {
        ...
        const pathFloor = selectedBuilding?.floors.find(f => f.id === path.floorId);
        if (pathFloor) {
  ...
  const handlePathColorChange = (pathId: string, color: string) => {
    setPaths(paths.map(path => 
      path.id === pathId ? { ...path, color } : path
    ));
  };
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {isBuildingMode ? "Building Management" : 
                   isPreviewMode ? "Preview Mode" : 
                   isTagMode ? "Location Tagging" :
                   isVerticalTagMode ? "Vertical Connector Tagging" :
                   isCreatingMultiFloorPath ? "Multi-Floor Path Designer" : "Interactive Path Designer"}
                  {isPublished && !isPreviewMode && !isBuildingMode && " (Published)"}
                </p>
              </div>
            </div>
            <div className="flex items-center space-x-2 text-sm text-gray-600">
              ...
              <span>
                ...
                {paths.length} paths • {tags.length} tags • {verticalConnectors.filter(c => c.floorId === selectedFloor?.id).length} vertical connectors
              </span>
              {isPublished && (
          ...
          <div className="text-center py-12">
            <div className="max-w-md mx-auto">
              ...
              <div className="flex space-x-4">
                ...
                <Button onClick={toggleBuildingMode} variant="outline" className="mt-4">
                ...
                {selectedBuilding && selectedFloor && (
                  <div className="p-4 border-b bg-gray-50">
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="font-medium text-gray-900">{selectedBuilding.name}</h3>
                        <p className="text-sm text-gray-500">Current floor: {selectedFloor.label}</p>
                        {isCreatingMultiFloorPath && (
                          <p className="text-sm text-blue-600 font-medium">
                            Multi-floor path mode active • Click near connectors to switch floors
                          </p>
                        )}
                      </div>
                      <div className="flex space-x-2">
                        ...
                        {selectedBuilding.floors.length > 1 && !isCreatingMultiFloorPath && (isDesignMode || isEditMode) && (
                          <Button
                            variant="outline"
                            size="sm"
                    ...
                    <MapCanvas
                      ...
                      paths={(() => {
                        ...
                        const floorFilteredPaths = paths.filter(path => {
                          if (path.isMultiFloor && path.segments) {
                            return path.segments.some(segment => segment.floorId === selectedFloor?.id);
                          }
                          return !selectedFloor?.id || path.floorId === selectedFloor.id || !path.floorId;
                        });
                        ...
                        if (isPreviewMode) {
                          if (selectedPathForAnimation && animatedPath) {
                            return floorFilteredPaths.filter(path => 
                      ...
                      tags={tags.filter(tag => !selectedFloor?.id || tag.floorId === selectedFloor.id || !tag.floorId)}
                      verticalConnectors={verticalConnectors.filter(c => c.floorId === selectedFloor?.id)}
            ...
            <div className="lg:col-span-1 space-y-4">
              {isPreviewMode ? (
                ...
                <>
                  ...
                  />
                  {tags.filter(tag => !selectedFloor?.id || tag.floorId === selectedFloor.id || !tag.floorId).length > 0 && (
                    <ColorCustomizer
                      tags={tags.filter(tag => !selectedFloor?.id || tag.floorId === selectedFloor.id || !tag.floorId)}
                      paths={[]}
                ...
                <>
                  ...
                  />
                  {(tags.filter(tag => !selectedFloor?.id || tag.floorId === selectedFloor.id || !tag.floorId).length > 0 || paths.length > 0) && (
                    <ColorCustomizer
                      tags={tags.filter(tag => !selectedFloor?.id || tag.floorId === selectedFloor.id || !tag.floorId)}
                      paths={paths}
                      onTagColorChange={handleTagColorChange}
