...
const Page: React.FC = () => {
  ...
  const fetchOrgOfSuperAdmin = useCallback(async () => {
    try {
      const orgs = await OrgOfSuperAdmin();
      setOrgOfSuperrAdmin(orgs as SuperAdminOrg);
    } catch (error) {
  ...
  }, [fetchOrganizations, fetchAllUsers, fetchAllRole, fetchOrgOfSuperAdmin]);
  ...
  const confirmDeleteUserOrAdmin = async () => {
    const { userId } = deleteUserConfirmation;

    if (!userId) return;

    const success = await deleteUserOrAdmin(userId);
    if (success) {
