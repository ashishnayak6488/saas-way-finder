...
export const AuthProvider = ({ children }: AuthProviderProps): JSX.Element => {
      ...
      .then((data) => {
        ...
        if (data.isLoggedIn && data.user) {
          ...
          const normalizedUser: User = {
            id: data.user.user_uuid || data.user.id,
            username: data.user.username || data.user.email,
            email: data.user.email || data.user.username,
            role_id: String(data.user.role_id), // Ensure it's a string
            user_uuid: data.user.user_uuid,
            first_name: data.user.first_name,
            last_name: data.user.last_name,
            entity_key: data.user.entity_key,
            entity_uuid: data.user.entity_uuid,
  ...
  useEffect(() => {
    if (authChecked && !loading) {
      // Only check auth again if we're navigating to a protected route and not authenticated
      if (!isPublicRoute(pathname) && !isAuthenticated) {
        checkAuth();
      }
    }
  }, [pathname, authChecked, isAuthenticated, loading]);
  ...
  const login = async (credentials: Credentials): Promise<LoginResponse> => {
    try {
      ...
      // Perform auth check to get complete user data
      await checkAuth();
  ...
  const logout = async (): Promise<boolean> => {
    try {
      ...
      // router.push("/login");
      window.location.href = "/login"; // Use window.location.href to ensure redirect works in all cases
      showToast.success("Logged out successfully");
      return true;
    } catch (error) {
      console.error("Logout error:", error);
